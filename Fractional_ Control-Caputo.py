...
##############################################################################
## Control T Lysis of glioma U87 cells
##############################################################################
def D(C,G,w):
    d= 0.1222**θ#0.06560705**θ
    s=  0.4042  **θ# 0.15297576  **θ
    l=1.4076**θ#  1.57998522**θ
    return d*C**l/(w**θ+s*G**l+C**l)
    

#w describe the search time which is incorporated to represents tumor cell lysis by Control T cells
      #When the search time is positive, the effectiveness of tumour cell lysis decrease


    #w:  0.10000426 (init = 0.1)
...
def eq(par,initial_cond,start_time,end_time,incr):
    ...
    def funct(y,t):
        ...
        #μ_2,e_1,μ_T,b_1,α_M=par
        #α_D,K,θ_M,μ_2,e_1,μ_T,b_1,α_M=par
        θ_M,α,μ_2,e_1,μ_T,α_M=par
        DD=D(C_T,G,w= 0.10000426)#0.0001#.3251;0.24
        #dos=dosage
...
def eq2(par,initial_cond,start_time,end_time,incr):
    ...
    def funct(y,t):
        ...
        G,C_T,C_M,F,T_β,M = y
        θ_M,α,μ_2,e_1,μ_T,α_M=par
        DD=D(C_T,G,w= 0.10000426)#0.0001 w=.3251;0.24
        #dos=dosage
        ####################################################        
        ####################################################
...
obs=100
for θ in [0.7715522030079129]:#θ=0.7715522030079129
    ...
    #NIC=[.17,Cd[0],0.001,0.0001,0.0001,3.009e-6]
    ...
    plt.plot(T, F1, 'g-', linewidth=2)
    
    start_time=T[-1]
    end_time=T[-1]+25
    NIC1=[F0[-1],.10,F2[-1],F3[-1],F4[-1],F5[-1]]
    F0,F1,F2,F3,F4,F5,T=eq2(newrates,NIC1,start_time,end_time,intervals)
    plt.plot(T, F1, 'g-', linewidth=2,label='Small dose')
    ...
    start_time=T[-1]
    end_time=T[-1]+25
    NIC1=[F0[-1],1,F2[-1],F3[-1],F4[-1],F5[-1]]
    F0,F1,F2,F3,F4,F5,T=eq2(newrates,NIC1,start_time,end_time,intervals)
    plt.plot(T, F1, 'r-', linewidth=2,label='Medium dose')
    ...
    YY0=[0.05,0,0.0000,0,1e-7,0.000]
    F0,F1,F2,F3,F4,F5,T=eq2(newrates,YY0,start_time,end_time,intervals)
    plt.plot(T, F1, 'b-', linewidth=2)
    
    start_time=T[-1]
    end_time=T[-1]+25
    NIC1=[F0[-1],10,F2[-1],F3[-1],F4[-1],F5[-1]]
    F0,F1,F2,F3,F4,F5,T=eq2(newrates,NIC1,start_time,end_time,intervals)
    plt.plot(T, F1, 'b-', linewidth=2,label='maximum dose')
